...
import { Edit, Trash2, PlusCircle, ArrowLeft, Package, Building2, Users, DollarSign, Scale, TrendingUp, Search, Briefcase } from 'lucide-react';
...
const ProjectMaterialMapping = () => {
    ...
    // 1. Fetch All Projects for Dropdown
    useEffect(() => {
    ...
    // 2. Fetch Data when Project is Selected
    const fetchProjectAndMappings = useCallback(async (id) => {
        if (!id) return;
        ...
        try {
            ...
            const [projectResponse, mappingsResponse] = await Promise.all([
                ...
                fetch(`${API_BASE_URL}/projectMaterialMappings/project/${id}`, {
                    headers: { 'Authorization': `Bearer ${token}` }
                })
            ]);
    ...
    useEffect(() => {
        if (selectedProjectId) {
            fetchProjectAndMappings(selectedProjectId);
        } else {
            setProject(null);
            setMappings([]);
        }
    }, [selectedProjectId, fetchProjectAndMappings]);
    ...
    const handleAdd = () => {
        if (!selectedProjectId) {
            setMessageBox({ type: 'error', message: 'Please select a project first.' });
            return;
        }
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-gray-50 to-blue-50 p-6">
            <div className="max-w-7xl mx-auto">
                {/* Header Section - ALL IN LEFT SIDE */}
                <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-8 gap-6">
                    <div className="flex items-center space-x-4">
                        ...
                        <div>
                            ...
                            <p className="text-gray-600 mt-1">Manage materials for {project?.projectName || 'Selected Project'}</p>
                        </div>
                    </div>

                    {/* Add Material Button - RIGHT SIDE */}
                    {selectedProjectId && !dataLoading && (
                        <button
                            onClick={handleAdd}
                            className="flex items-center px-6 py-3 bg-gradient-to-r from-green-500 to-emerald-600 hover:from-green-600 hover:to-emerald-700 text-white font-semibold rounded-xl transition-all duration-200 transform hover:scale-[1.02] shadow-lg hover:shadow-xl ml-auto"
                        >
                ...
                {/* Project Selection Section - BELOW HEADER, LEFT SIDE */}
                <div className="mb-6">
                    <div className="relative max-w-md">
                        ...
                        <select
                            ...
                            className="w-full pl-10 pr-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-transparent bg-white shadow-sm text-gray-700 font-medium transition-all duration-200"
                        >
                ...
                {!selectedProjectId ? (
                    <div className="flex flex-col items-center justify-center bg-white rounded-xl shadow-lg border border-gray-100 p-12 text-center">
                        <Briefcase className="h-16 w-16 text-indigo-200 mb-4" />
                        <h3 className="text-xl font-semibold text-gray-700 mb-2">No Project Selected</h3>
                        <p className="text-gray-500 max-w-md">Please select a project from the dropdown above to view its material mappings.</p>
                    </div>
                ) : dataLoading ? (
                                        ...
                                        <tbody className="bg-white divide-y divide-gray-100">
                                            {filteredMappings.map((mapping) => (
                                                <tr key={mapping._id} className="hover:bg-indigo-50/30 transition-all duration-150">
                                                    ...
                                                    <td className="px-6 py-4 whitespace-nowrap">
                                                        <div className={`text-sm font-bold px-3 py-2 rounded-lg ${mapping.balanceQuantity > 0
                ...
                {showFormModal && (
                    <Modal onClose={() => handleFormUpdate('cancel')}>
                        {/* ProjectId prop removed from here as it's handled by state */}
